    ...
    <article className={`word-card ${compact ? "compact" : ""}`}>
      ...
      {expanded && <div>
        ...
        <div className="word-details" style={{ display: "block" }}>
          ...
          {catalogPhrases.length ? <div className="detail-group"><span className="detail-label">PHRASES IN QUIZ</span>{catalogPhrases.map((seed) => { const phraseExamples = [{ english: seed.exampleEnglish, japanese: seed.exampleJapanese }, ...(phraseExampleCatalog[seed.id] ?? [])]; return <div key={seed.id} style={{ marginBottom: "10px" }}><p><strong>{seed.expression}</strong><em>{seed.meaningJa}</em></p>{phraseExamples.map((example, index) => <div key={`${seed.id}-example-${index}`} style={{ position: "relative", paddingRight: "42px" }}><span className="detail-label" style={{ marginBottom: "4px" }}>例文{index + 1}</span><p className="example-en">{example.english}</p><p className="example-ja">{example.japanese}</p><SpeakButton text={example.english} label={`${seed.expression}の例文${index + 1}を再生`} /></div>)}</div>; })}</div> : null}
