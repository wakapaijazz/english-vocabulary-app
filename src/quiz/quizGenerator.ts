...
function getMeaning(entry: VocabularyEntry): string {
  return primarySense(entry).meaningJa.trim();
}

function getDefinition(entry: VocabularyEntry): string {
  return primarySense(entry).meaningEn?.trim() || definitionCatalog[entry.lemma] || "to have a particular meaning or use";
}
...
function findClozeTarget(lemma: string, english: string, preferred?: string): string | undefined {
  const candidates = [
    preferred,
    lemma,
    lemma.endsWith("y") ? `${lemma.slice(0, -1)}ied` : undefined,
    lemma.endsWith("y") ? `${lemma.slice(0, -1)}ies` : undefined,
...
function prepareExamples(entry: VocabularyEntry): PreparedExamples {
  ...
  const direct = [...directExamples, ...(supplement ? [supplement] : [])]
...
function baseQuestion(
  ...
  return {
    ...
    senseId: primarySense(entry).id,
    type,
    skill: type === "ja-to-en" ? "production" : type === "cloze" ? "context" : "recognition",
    prompt: "",
    choices: [],
    correctChoiceId: "",
...
function articleFor(word: string): "a" | "an" {
  ...
  const anSound = new Set(["honest", "honor", "honour", "hour", "heir", "herb"]);
  const aSound = new Set(["european", "ewe", "unicorn", "unique", "unit", "unified", "university", "utility", "user", "usual", "useful", "one"]);
  if (anSound.has(lower)) return "an";
  if (aSound.has(lower)) return "a";
...
function createWordQuestion(entry: VocabularyEntry, entries: VocabularyEntry[], type: QuestionType, index: number): QuizQuestion {
  ...
  if (type === "en-to-ja") {
    question.prompt = entry.lemma;
    question.choices = createChoices(entry, entries, meaningJa, "meaning").map((choice) =>
      choice.text === meaningJa ? { ...choice, id: correctChoiceId } : choice,
    );
  } else if (type === "en-to-en") {
    question.prompt = definitionEn;
    question.choices = createChoices(entry, entries, entry.lemma, "lemma").map((choice) =>
      choice.text === entry.lemma ? { ...choice, id: correctChoiceId } : choice,
    );
  } else if (type === "ja-to-en") {
    question.prompt = meaningJa;
    question.choices = createChoices(entry, entries, entry.lemma, "lemma").map((choice) =>
      choice.text === entry.lemma ? { ...choice, id: correctChoiceId } : choice,
    );
  } else {
    ...
    question.choices = rawChoices.map((choice) => {
      const text = articleMatch ? `${articleFor(choice.text)} ${choice.text}` : choice.text;
      return choice.text === entry.lemma ? { ...choice, text, id: correctChoiceId } : { ...choice, text };
    });
  }
...
function createPhraseQuestion(seed: PhraseQuestionSeed, index: number, entries: VocabularyEntry[]): QuizQuestion {
...
function eligibleEntries(entries: VocabularyEntry[], mode: QuizMode, history: LearningHistory = {}, level?: number): VocabularyEntry[] {
  const byLevel = entries.filter((entry) => level === undefined || entry.level === level);
  const filtered = byLevel.filter((entry) => {
    const state = history[entry.id];
    if (mode === "new") return !state?.seenCount;
    if (mode === "review") return Boolean(state && isDue(state));
    if (mode === "mistakes") return Boolean(state?.wrongCount);
    if (mode === "weak") return Boolean(state && isWeak(state));
    return true;
  });
  return filtered.length ? filtered : byLevel;
}

export function generateQuizQuestions(
  entries: VocabularyEntry[],
  options: { type: QuizType; count: number; mode?: QuizMode; history?: LearningHistory; level?: number },
): QuizQuestion[] {
  const pool = shuffle(eligibleEntries(entries, options.mode ?? "all", options.history, options.level));
  if (!pool.length || options.count <= 0) return [];
  const phrasePool = shuffle(phraseCatalog);
  let phraseIndex = 0;

  return Array.from({ length: options.count }, (_, index) => {
    const entry = pool[index % pool.length];
    const type = options.type === "mixed" ? mixedTypes[index % mixedTypes.length] : options.type;
    if (type === "collocation") {
      const seed = phrasePool[phraseIndex % phrasePool.length];
      phraseIndex += 1;
      return createPhraseQuestion(seed, index, entries);
    }
    return createWordQuestion(entry, entries, type, index);
  });
}

export function validateQuizQuestion(question: QuizQuestion): boolean {
  const choiceIds = new Set(question.choices.map((choice) => choice.id));
  return question.choices.length === 4
    && choiceIds.size === 4
