...
function createPhraseQuestion(seed: PhraseQuestionSeed, index: number, entries: VocabularyEntry[]): QuizQuestion {
  const relatedEntry = entries.find((entry) => entry.lemma === seed.word); const meanings = phraseMeaningCatalog[seed.expression] ?? {}; const correctChoiceId = `phrase-choice-${seed.id}-correct`; const extraExamples = phraseExampleCatalog[seed.id]?.filter((example) => example.english.trim() && example.japanese.trim()) ?? []; const availableExamples = extraExamples.length ? extraExamples : [{ english: seed.exampleEnglish, japanese: seed.exampleJapanese }]; const selectedExample = pick(availableExamples);
