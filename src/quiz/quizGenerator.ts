...
import { phraseMeaningExpansion3 } from "../data/phraseMeaningExpansion3";
import { phraseMeaningExpansion4 } from "../data/phraseMeaningExpansion4";
import { phraseMeaningExpansion5 } from "../data/phraseMeaningExpansion5";
...
function createPhraseQuestion(seed: (typeof allPhraseCatalog)[number], index: number, entries: VocabularyEntry[]): QuizQuestion {
  const relatedEntry = entries.find((entry) => entry.lemma === seed.word); const meanings = { ...(phraseMeaningCatalog[seed.expression] ?? {}), ...(phraseMeaningExpansion[seed.expression] ?? {}), ...(phraseMeaningExpansion2[seed.expression] ?? {}), ...(phraseMeaningExpansion3[seed.expression] ?? {}), ...(phraseMeaningExpansion4[seed.expression] ?? {}), ...(phraseMeaningExpansion5[seed.expression] ?? {}) }; const correctChoiceId = `phrase-choice-${seed.id}-correct`; const availableExamples = getPhraseExamples(seed); const selectedExample = pick(availableExamples);
  return { id: `phrase-${seed.id}-${index}`, vocabularyId: relatedEntry?.id ?? seed.id, favoriteId: `phrase:${seed.id}`, senseId: relatedEntry?.senses[0]?.id, type: "collocation", skill: "collocation", prompt: selectedExample.prompt ?? seed.prompt, choices: seed.choices.map((choice, choiceIndex) => ({ id: choice.text === seed.answer ? correctChoiceId : `phrase-choice-${seed.id}-${choiceIndex}`, text: choice.text, meaningJa: meanings[choice.text] ?? choice.meaningJa })), correctChoiceId, explanation: selectedExample.japanese, details: { word: seed.expression, meaningJa: seed.meaningJa, definitionEn: seed.definitionEn, exampleEnglish: selectedExample.english, exampleJapanese: selectedExample.japanese, examples: availableExamples.map(({ english, japanese }) => ({ english, japanese })) } };
}
